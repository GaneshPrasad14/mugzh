...
const Checkout = () => {
  ...
  const handleCompleteOrder = async () => {
    ...
    const orderData = {
      customerDetails,
      shippingAddress,
      billingDetails: hasSeparateBilling ? billingDetails : shippingAddress,
      items: cartItems,
      subtotal,
    ...
    if (paymentMethod === "cod") {
      try {
        const res = await fetch("http://localhost:5002/api/orders/cod", {
          method: "POST",
      ...
      try {
        ...
        const options = {
          key: "rzp_live_SLDKjL9wgU6S7I", // User's live key
          ...
          handler: async function (response: any) {
            // Verify Payment
            const verifyRes = await fetch("http://localhost:5002/api/orders/verify", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                ...
                {!paymentComplete ? (
                  <div className="space-y-6">
                    <RadioGroup
                      ...
                      className="space-y-4"
                    >
                      <div className="flex items-center justify-between p-4 border border-muted-foreground/20 rounded-none cursor-pointer" onClick={() => setPaymentMethod("razorpay")}>
                        <div className="flex items-center space-x-3">
                          <RadioGroupItem value="razorpay" id="razorpay" />
                          <Label htmlFor="razorpay" className="font-light text-foreground cursor-pointer">
                      ...
                      <div className="flex items-center justify-between p-4 border border-muted-foreground/20 rounded-none cursor-pointer" onClick={() => setPaymentMethod("cod")}>
                        <div className="flex items-center space-x-3">
                          <RadioGroupItem value="cod" id="cod" />
                          <Label htmlFor="cod" className="font-light text-foreground cursor-pointer">
